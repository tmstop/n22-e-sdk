...
void IIC_Init(void)
{			
  GPIO_REG(GPIO_OUTPUT_EN)  |= ((0x1 << IIC_SDA_GPIO_OFFSET )|(0x1 << IIC_SCL_GPIO_OFFSET ));
  GPIO_REG(GPIO_INPUT_EN)   &= ~ ((0x1 << IIC_SDA_GPIO_OFFSET)|(0x1 << IIC_SCL_GPIO_OFFSET ));
  GPIO_REG(GPIO_OUTPUT_VAL)  |=  ((0x1 << IIC_SDA_GPIO_OFFSET)|(0x1 << IIC_SCL_GPIO_OFFSET )) ;   
}



void IIC_SCL(__uint8_t GPIOVAL)
{
	if(GPIOVAL==1)
		GPIO_REG(GPIO_OUTPUT_VAL)  |=  (0x1 << IIC_SCL_GPIO_OFFSET);
	else
		GPIO_REG(GPIO_OUTPUT_VAL)  &= ~(0x1 << IIC_SCL_GPIO_OFFSET);	
}
void IIC_SDA(__uint8_t GPIOVAL)
{
	if(GPIOVAL==1)
		GPIO_REG(GPIO_OUTPUT_VAL)  |=  (0x1 << IIC_SDA_GPIO_OFFSET);
	else
		GPIO_REG(GPIO_OUTPUT_VAL)  &= ~(0x1 << IIC_SDA_GPIO_OFFSET);	
}
/**************************实现函数********************************************
...
*功　　能:	    IIC发送一个字节
*******************************************************************************/		  
void IIC_Send_Byte(__uint8_t txd)
{                        
    __uint8_t t;   
	SDA_OUT(); 	    
...
*******************************************************************************/  
__uint8_t IIC_Read_Byte(unsigned char ack)
{
...
返回   1
*******************************************************************************/ 
__uint8_t IICreadByte(__uint8_t dev, __uint8_t reg, __uint8_t *data){
...
__uint8_t IICwriteBits(__uint8_t dev,__uint8_t reg,__uint8_t bitStart,__uint8_t length,__uint8_t data)
{
    ...
    if (IICreadByte(dev, reg, &b) != 0) {
        __uint8_t mask = (0xFF << (bitStart + 1)) | 0xFF >> ((8 - bitStart) + length - 1);
        data <<= (8 - length);
        data >>= (7 - bitStart);
        b &= mask;
        b |= data;
